# =============================================================================
# Step 6: Pseudotime Analysis with Monocle3 (Multiple Subdata)
# =============================================================================
# This script performs pseudotime analysis using monocle3 for target cell types
# across multiple subdata sets for comparative analysis.
# =============================================================================
...
# ---------------------------
# Multiple subdata selection for combined analysis
# ---------------------------
SUBDATA_AUTHORS <- c("Goswami2019", "Sankowski2019", "Pombo2021")  # Multiple datasets to combine
SUBDATA_SUFFIX <- "GSP_TAMs"  # Abbreviated name for output files and directories
...
# Create multi-subdata folder structure
...
# Apply multi-subdata filtering
if (!("author" %in% colnames(seurat_obj@meta.data))) {
  stop("'author' column not found in meta.data. Available columns: ", paste(colnames(seurat_obj@meta.data), collapse = ", "))
}

available_authors <- unique(seurat_obj@meta.data$author)
cat("Available authors:", paste(available_authors, collapse = ", "), "\n")

# Check if all requested authors exist
missing_authors <- setdiff(SUBDATA_AUTHORS, available_authors)
if (length(missing_authors) > 0) {
  warning("Some requested authors not found in data: ", paste(missing_authors, collapse = ", "))
  SUBDATA_AUTHORS <- intersect(SUBDATA_AUTHORS, available_authors)
}

if (length(SUBDATA_AUTHORS) == 0) {
  stop("No valid authors found for filtering")
}

# Filter to selected authors
original_cells <- ncol(seurat_obj)
seurat_obj <- subset(seurat_obj, subset = author %in% SUBDATA_AUTHORS)
cat("Filtered to", ncol(seurat_obj), "cells from", original_cells, "total (authors:", paste(SUBDATA_AUTHORS, collapse = ", "), ")\n")
...
analysis_summary <- data.frame(
  Subdata_Authors = paste(SUBDATA_AUTHORS, collapse = ", "),
  Subdata_Suffix = SUBDATA_SUFFIX,
...
cat("Subdata authors:", paste(SUBDATA_AUTHORS, collapse = ", "), "\n")
cat("Subdata suffix:", SUBDATA_SUFFIX, "\n")
cat("Summary:\n")
